...
# This download didn't work. Instead, I cloned the 'restricted data' folder on GitHub to my local machine.
## I made a copy of that data and moved the file into RPr-Malcomb-2014/data/scratch
...
#   )
# modified download code 
## getting a 404 page not found error
# if (!"MWI_adm2.shp" %in% list.files(private_r)) {
#   download(
#     "https://github.com/opengisci/restricted-data/blob/main/MWI_adm2.zip",
#     here(scratch, "MWI_adm2_shp.zip")
#   )
...
Calculate vulnerability score using geometric aggregation.
$$
Vulnerability = (40 - Adaptive Capacity) * Livelihood Sensitivity * Drought Exposure * Flood Risk
$$
```{r geometric-combined-vulnerability}
# final output (multiply component rasters)
vulnerability_rg <- (40 - capacity_r) * sensitivity_r * drought_r * flood_r 
# 100 = highest vulnerability, 0 = lowest vulnerability
names(vulnerability_rg) <- "vulnerability (geometric aggregation)"
plot(vulnerability_rg)
```
 
## Figure 6: Geometric Vulnerability 

```{r find break & limit values for geometric vulnerability map}
g_vuln_max <- max(vulnerability_rg[[1]],na.rm = TRUE)
g_vuln_min <- min(vulnerability_rg[[1]],na.rm = TRUE)
```

```{r geometric-vulnerability-map}
g_vuln_map = ggplot() +
  geom_stars(data = vulnerability_rg) +
  scale_fill_gradient(
    low = "#FFFF75",
    high = "#CF4611",
    breaks = c(g_vuln_min, g_vuln_max),
    labels = c("Lower Vulnerability", "Higher Vulnerability"),
    na.value = "transparent",
    guide = "colourbar",
    limits = c(g_vuln_min, g_vuln_max)
  ) +
  labs(title = "Malawi Vulnerability to Climate Change") +
  theme_minimal() +
  theme(
...
# Conclusions
In this study, we were partially able to reproduce the original study's map of household resilience as a function of assets and access, but we were unable to reproduce their map of overall vulnerability. One of the goals of the original paper was to "determine geographic areas most susceptible to hazards, climatic change and vulnerability, and degraded livelihood security" in order to inform pragmatic allocation of climate change adaptation funding. The results of this reproduction study indicate that the original study could be made stronger by providing further explanation of its methods and decision-making process. In turn, this would allow for future reproduction and replication studies to test the precision of the original study as well as new methods to improve its accuracy. Furthermore, it is vital that the original study be reproducible seeing as Malawi has a rapidly growing population...

that went into  For the findings of the original paper to be most effectively applied in identifying priority areas for adaptation funding, where funding might be best spent in addressing climate vulnerability, it is essential that the results of the paper be verified -> for now; and replicable -> for the future. additional info needed re: fieldwork, even though that was beyond the scope of this study 
